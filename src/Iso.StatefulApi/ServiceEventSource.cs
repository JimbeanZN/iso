...
namespace Iso.StatefulApi
{
	[EventSource(Name = "MyCompany-Iso-StatefulApi")]
	internal sealed class ServiceEventSource : EventSource
	{
		...
		{
#if !UNSAFE
			WriteEvent(ServiceMessageEventId, serviceName, serviceTypeName, replicaOrInstanceId, partitionId, applicationName,
...
SizeInBytes(applicationTypeName) };
		...
		[Event(ServiceTypeRegisteredEventId, Level = EventLevel.Informational,
			Message = "Service host process {0} registered service type {1}", Keywords = Keywords.ServiceInitialization)]
